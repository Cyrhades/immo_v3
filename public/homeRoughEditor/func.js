...
function initHistory(boot = false) {
    HISTORY.index = 0;
    if(document.querySelector("#history") != null && document.querySelector("#history").value != '') {
        //HISTORY.push(JSON.parse(document.querySelector("#history").value));
        HISTORY[0] = document.querySelector("#history").value;
        localStorage.setItem('history', JSON.stringify(HISTORY));
        load(0);
        save();
    }
    else if (!boot && localStorage.getItem('history')) localStorage.removeItem('history');
...
function zoom_maker(lens, xmove, xview) {
    ...
    if(isNaN(width_viewbox)) width_viewbox = taille_w;
    if(isNaN(height_viewbox)) height_viewbox = taille_h;
    if(isNaN(ratio_viewbox)) ratio_viewbox = height_viewbox / width_viewbox;
    ...
    else if (lens == 'zoomin' && zoom < 14 && zoom > 0) {
        zoom++;
    ...
    if((isNaN(originX_viewbox) || isNaN(originY_viewbox) || isNaN(width_viewbox) || isNaN(height_viewbox)) !== true) {
      $('svg').each(function() {
          $(this)[0].setAttribute('viewBox', originX_viewbox + ' ' + originY_viewbox + ' ' + width_viewbox + ' ' + height_viewbox)
      });
    }
...
function cursor(tool) {
  if (tool == 'grab') tool = "url('/homeRoughEditor/add.png') 8 8, auto";
  if (tool == 'scissor') tool = "url('/homeRoughEditor/hand_scissors-64.png'), auto";
  if (tool == 'trash') tool = "url('/homeRoughEditor/Cancel-32.png'), auto";
  if (tool == 'validation') tool = "url('/homeRoughEditor/check.png'), auto";
  $('#lin').css('cursor',tool);
}
